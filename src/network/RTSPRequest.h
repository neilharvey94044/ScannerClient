...
class RTSPRequest {
    ...
    protected:
        ...
        string rtspEnd              {"\r\n\r\n"};
        string rtspCR               {"\r\n"};
        string rtspAttrAccept       {"Accept: application/sdp"};
        string rtspAttrTransport    {"Transport: RTP/AVP;unicast;client_port={}"};
        string rtspAttrSession      {"Session: {}"};
        string rtspAttrRange        {"Range: npt=0.000-"};
        string rtspHdr              {"{} rtsp://{}{}/au:scanner.au{} RTSP/1.0"+ rtspCR +"CSeq: {}"+rtspCR+"User-Agent: {}"};
